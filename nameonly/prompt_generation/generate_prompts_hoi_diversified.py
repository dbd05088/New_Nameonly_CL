...
def generate_prompt(client, object, action, previous_prompt_list):
    base_message = f"To generate images using a text-to-image generation model, I need to create a prompt. Below, a pair of (object, action) will be provided, and the goal is to generate a prompt using the object and action pair. For example, if the object is 'dog' and the action is 'run', the prompt could be 'Dogs are running.', 'Dogs are sprinting,' 'Dogs are dashing,' 'Dogs are moving quickly,' etc. Here is a list of prompts that I have previously generated. Please create a new prompt that does not overlap with these."
    end_message = f"\nobject: {object}, action: {action}. Please create one prompt sentence (under 10 words) that fits this description. Please ensure the response format is strictly 'prompt: answer'.\n"
...
prompt_json_path = './prompts/hoi_diversified.json'
...
for dataset in tqdm(dataset_list):
    data_dict = {}
    object_name = dataset[0]; action_name = dataset[1]
    primary_prompt = dataset[2]
    count = dataset[3]
    previous_prompt_list = []
    
    for i in enumerate(tqdm(range(MAX_PROMPT_NUM - 1))):
        try:
            prompt = generate_prompt(client, object_name, action_name, previous_prompt_list)
            print(f"Previous prompt list: {previous_prompt_list}")
            print(f"Generated prompt: {prompt}")
            previous_prompt_list.append(prompt)
        except Exception as e:
            breakpoint()
            pass
    
    previous_prompt_list.insert(0, primary_prompt)
    
    print(f"Object: {object_name}, Action: {action_name}, Primary: {primary_prompt}, Prompts: {previous_prompt_list}")
    data_dict = {
        'object_name': object_name,
        'action_name': action_name,
        'prompts': previous_prompt_list,
        'count': count
    }
