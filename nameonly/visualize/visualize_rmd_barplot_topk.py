...
# 둘이 top-동일 개수(ex. 5, 10 20...)개씩 비교해서 RMD score의 average를 구하는거 어떨까요?
...
pickle_path = 'RMD_scores/RMD_scores_cifar10_ablation_equalsize_with_rmd_emsembled.pkl'
count = 20
...
model_class_dict = {}; model_class_dict_ensemble = {}
...
for class_name in cls_model_scores_dict:
    ...
    base_scores = model_scores_dict['cifar10_base_1x']
    base_scores = [x[1] for x in base_scores]
    full_scores = model_scores_dict['cifar10_full']
    full_scores = [x[1] for x in full_scores]
    ensemble_scores = model_scores_dict['cifar10_ensembled']
    ensemble_scores = [x[1] for x in ensemble_scores]
    
    # Get top-k scores
    base_scores.sort(reverse=True); full_scores.sort(reverse=True); ensemble_scores.sort(reverse=True)
    base_scores = base_scores[:count]; full_scores = full_scores[:count]; ensemble_scores = ensemble_scores[:count]
    base_average = sum(base_scores) / len(base_scores); full_average = sum(full_scores) / len(full_scores); ensemble_average = sum(ensemble_scores) / len(ensemble_scores)
    
    result_dict = {'cifar10_base_1x': base_average, 'cifar10_full': full_average, 'cifar10_ensembled': ensemble_average}
...
base_average = [cls_model_count_dict[cls]['cifar10_base_1x'] for cls in cls_mapping_list]
full_average = [cls_model_count_dict[cls]['cifar10_full'] for cls in cls_mapping_list]
ensemble_average = [cls_model_count_dict[cls]['cifar10_ensembled'] for cls in cls_mapping_list]
base_average.append(sum(base_average) / len(base_average))
full_average.append(sum(full_average) / len(full_average))
ensemble_average.append(sum(ensemble_average) / len(ensemble_average))
...
x_full = x_base_1x + bar_width
x_ensembled = x_base_1x + 2 * bar_width
...
axs.bar(x_ensembled, ensemble_average, width=bar_width, label="CIFAR-10 RMD Ensembled", alpha=0.9, color="#FBE5D6", edgecolor="k")
axs.set_xticks(x_full)
axs.yaxis.set_tick_params(labelsize=35)
axs.set_xticklabels(classes, fontsize=30, rotation=45)
axs.set_title(f'Average RMD scores of top {str(count)} samples', fontsize=45)
# axs.set_ylim(4,10)

# Add RMD count plot
axs.set_ylabel("Average RMD score", fontsize=30)
# line1 = axs2.plot(x_full - 0.35, base_count, label="CIFAR-10 Base Prompt", color="red", marker='o', linewidth=3)
# line2 = axs2.plot(x_full, full_count, label="CIFAR-10 Prompt Rewrites", color="pink", marker='o', linewidth=3)
