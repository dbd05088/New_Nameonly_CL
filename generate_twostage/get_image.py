...
def adjust_list_length(lst, length):
...
def generate_single_class(
    ...
    concatenated_prompt_list = adjust_list_length(concatenated_prompt_list, num_samples)
    ...
    for i, (prompt, prompt_type, image_name) in enumerate(tqdm(concatenated_prompt_list)):
        ...
        # info_dict[image_name] = {'type':prompt_type, 'prompt': prompt}
    ...
    # for prompt_idx, prompt_name in enumerate(tqdm(prompt_list)):
    #     print(f"Generating images for prompt {prompt_idx + 1}/{len(prompt_list)}")
    #     print(f"Prompt: {prompt_name.replace('_', ' ')}")
        
    #     if generative_model == "DALLE2":
    #         images = model.generate_k_images(prompt_name.replace("_", " "), k=1)
    #         if images == None:
    #             print(f"Error generating images for {cls}")
    #             continue
    #         for j, img in enumerate(images):
    #             # Resize image to 224x224
    #             img = img.resize((224, 224))
    #             img.save(os.path.join(image_dir, cls, str(prompt_idx) + "_" + str(j) + ".jpg"), "JPEG")
    #     else:
    #         image = get_image(prompt_name, model)
    #         # Resize image to 224x224
    #         image = image.resize((224, 224))
    #         if type(image) == list:
    ...
    #                 img.save(os.path.join(image_dir, cls, str(prompt_idx) + "_" + str(j) + ".jpg"), "JPEG")
    #         else:
    #             image.save(os.path.join(image_dir, cls, str(prompt_idx) + ".jpg"), "JPEG")
