...
class REMIND(ER):
    ...
    def prep_pqtrain(self):
        print("prep for pq")
        train_df = pd.DataFrame(self.memory.images)
        ...
        self.pq_train_loader = DataLoader(train_dataset, batch_size=self.batch_size, shuffle=False)
    ...
    def base_initialize(self):
        ...
        start_ix = 0
        features_data = np.empty((len(self.pq_train_loader.dataset), self.num_channels, self.spatial_feat_dim, self.spatial_feat_dim), dtype=np.float32)
        labels_data = np.empty((len(self.pq_train_loader.dataset), 1), dtype=int)
        item_ixs_data = np.empty((len(self.pq_train_loader.dataset), 1), dtype=int)
    ...
    def fit_pq(self, feats_base_init, labels_base_init, item_ix_base_init, num_channels, spatial_feat_dim, num_codebooks, codebook_size, batch_size=128):
        ...
        for i in range(0, num_samples, self.batch_size):
            ...
            for i in range(len(codes)):
                ...
                # self.memory.replace_completed_sample(codes[i], batch_labels[i][0], finalize_baseinit=True)
    ...
    def online_train(self, iterations=1):
        ...
        for iter in range(iterations):
            ...
            if self.iteration == self.baseinit_iter_point:
                self.prep_pqtrain()
            if self.iteration == self.baseinit_iter_point:
