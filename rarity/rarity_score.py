...
class MANIFOLD(object):
	...
	def is_in_ball(self, k = 3, samples = None, cluster = False):
		...
		real2samples_distances = real2samples_distances.detach().cpu().numpy() # [num_real, num_generated]
		r = self.real2real_sorted[:,k] if not cluster else self.mode2mode_sorted[:,k] # (num_real,)
		# r: real sample들 기준으로 서로서로 거리를 재서, k번째로 가까운 거리를 kNN 거리로 정의

		# Radius vs distance
		dist_radi = (r[:,None].repeat(samples.shape[0], axis = 1) - real2samples_distances) # (num_real, num_generated)
		out_ball_ids = np.where((dist_radi > 0).any(axis = 0) == False)[0] # (471,)
