...
def generate_prompt_stage1(client):
    base_message = f"To generate images using a text-to-image generation model, I need to create a prompt. Keep the domain photorealistic and use different visual scenes and visual styles or different color profiles/ palettes."
...
def generate_prompt_stage2(client, metaprompt):
    base_message = f"To generate images using a text-to-image generation model, I need to create a prompt. Keep the domain photorealistic and use different visual scenes and visual styles or different color profiles/ palettes. The prompt should be similar to '{metaprompt}' but slightly different."
...
totalprompt_json_path = './prompts/static_totalprompts_with_cot_50_wo_cot.json'
...
#     try:
#         prompt = generate_prompt_stage1(client)
...
for i, metaprompt in enumerate(tqdm(metaprompt_list)):
    ...
    for j in range(num_prompts_per_metaprompt):
        try:
            prompt = generate_prompt_stage2(client, metaprompt)
