...
def save_dataset(dataset_name, input_folder, output_folder, subset_name, seed=None):
    ...
    for item in datalist:
        ...
        for idx in range(0, len(positive_files), num_per_set): #positive_imgfiles
            if idx + num_per_set >= len(positive_files):
                imgs = positive_files[-num_per_set:] + negative_files[-num_per_set:]
            else:
