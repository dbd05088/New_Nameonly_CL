...
class Ours(CLManagerBase):
    ...
    def initialize_future(self):
        ...
        if self.model_name == 'resnet18':
            ...
            self.target_layers = ["group1.blocks.block0.conv2.block.0.weight", "group1.blocks.block1.conv2.block.0.weight","group1.blocks.block2.conv2.block.0.weight", "group1.blocks.block3.conv2.block.0.weight", "group2.blocks.block0.conv2.block.0.weight", "group2.blocks.block1.conv2.block.0.weight","group2.blocks.block2.conv2.block.0.weight", "group2.blocks.block3.conv2.block.0.weight", "group3.blocks.block0.conv2.block.0.weight", "group3.blocks.block1.conv2.block.0.weight","group3.blocks.block2.conv2.block.0.weight", "group3.blocks.block3.conv2.block.0.weight"]
    ...
    def memory_future_step(self):
        ...
        if self.future_num_updates >= 1:
            if self.future_sample_num >= self.corr_warm_up and 'resnet' in self.model_name:
    ...
    def online_train(self, iterations=1):
        ...
        for i in range(iterations):
            ...
            if self.sample_num >= 2 and 'resnet' in self.model_name:
                    ...
                    if p.requires_grad is True and p.grad is not None:
                        if not p.grad.isnan().any():
                            if 'resnet' in self.model_name and i < self.num_blocks:
                                ...
                                if self.dataset == 'imagenet' and 'initial' in n:
                                    pool_factor = 4
                                else:
                                    pool_factor = 1
                                block_fisher[i] += (p.grad.clone().detach().clamp(-1, 1) ** 2).sum().item()/np.sqrt(get_group_attr(self.model).input_scale * pool_factor)
                                if self.unfreeze_rate < 1:
                                    self.total_flops += (len(p.grad.clone().detach().flatten())*2+get_group_attr(self.model).input_size*2) / 10e9
                            else:
...
class OurMemory(MemoryBase):
    def __init__(self, memory_size, T, count_decay_ratio, k_coeff, device='cpu'):
        ...
        self.retrieval_stats = {'freq': [], 'prob': [], 'cls_freq_sum': [], 'cls_freq_avg': [], 'cls_prob_sum': [], 'cls_prob_avg':[]}
