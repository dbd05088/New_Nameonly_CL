...
class ZeroShotClip:
    def __init__(self, train_datalist, test_datalist, device, **kwargs):
        ...
        self.model, self.pretrain_train_transform, self.pretrain_val_transform, self.tokenizer, self.criterion = select_model(self.model_name, self.pretrained_dataset)
    ...
    def report_test(self, domain_name, sample_num, avg_loss, avg_TR1, avg_TR5):
        writer.add_scalar(f"test/loss", avg_loss, sample_num)
        writer.add_scalar(f"test/acc", "avg_TR1", avg_TR1, "avg_TR5", avg_TR5, sample_num)
        logger.info(
    ...
    def online_evaluate(self, domain_name, cur_task, test_list, sample_num, batch_size, n_worker, cls_dict, cls_addition, data_time):
        ...
        self.report_test(domain_name, sample_num, eval_dict["avg_loss"], eval_dict["avg_TR1"], eval_dict["avg_TR5"])
            
        del test_loader
        return sample_num, eval_dict


    def evaluation(self, test_loader, criterion, downstream=True):
        total_correct, total_num_data, total_loss = 0.0, 0.0, 0.0
        ...
        with torch.no_grad():
            for i, data in enumerate(test_loader):
                ...
                image_features, text_features, logit_scale = self.model(imgs, texts)
        ...
        ret = {"avg_loss": avg_loss, "avg_TR1": avg_TR1, "avg_TR5": avg_TR5}
