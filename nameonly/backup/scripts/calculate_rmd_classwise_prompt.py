...
def calculate_features(image_paths):
...
if __name__ == "__main__":
    ...
    rmd_pickle_path = 'RMD_scores_PACS_prompt.pkl'

    image_path = "/workspace/home/user/seongwon/crawling/crawler/datasets/PACS/PACS_sdxl_prompt"

    classes = sorted(list(count_dict.keys()))
    prompts = [i for i in range(50)] # Prompt numbers
    images_dict = {prompt: {cls: [] for cls in classes} for prompt in prompts}
    for cls in classes:
        cls_path = os.path.join(image_path, cls)
        images = natsort.natsorted(os.listdir(cls_path))
        for prompt in prompts:
            # Prompt 0: 0_k.png, Prompt 1: 1_k.png, ...
            prompt_image_names = [image for image in images if image.split('_')[0] == str(prompt)]
            prompt_image_paths = [os.path.join(cls_path, image) for image in prompt_image_names]
            images_dict[prompt][cls] = prompt_image_paths
    ...
    for prompt in prompts:
        for cls, image_list in images_dict[prompt].items():
            class_labels += [cls] * len(image_list)
            concatenated_images += image_list
            prompt_labels += [prompt] * len(image_list)
    ...
    features = calculate_features(concatenated_images)
    ...
    # Split RMD scores using prompt labels
    RMD_each_prompt = {}
    for prompt in prompts:
        RMD_each_prompt[prompt] = RMD[prompt_labels == prompt]
    
    # Generate prompt - class - image_path - RMD dictionary
    prompt_class_image_RMD = {}
    for prompt in prompts:
        for cls in classes:
            indices = np.where((prompt_labels == prompt) & (class_labels == cls))[0]
            RMDs = RMD[indices]
            paths = np.array(concatenated_images)[indices]
            prompt_class_image_RMD[(prompt, cls)] = list(zip(paths, RMDs))

    # Save the RMD scores as pickle
    with open(rmd_pickle_path, 'wb') as f:
        pickle.dump(prompt_class_image_RMD, f)
