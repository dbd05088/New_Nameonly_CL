...
def get_image_paths_from_file(file_path, subset):
    count_dict = get_count_dict(subset)
    valid_labels = [i for i in range(len(count_dict))]
    ...
    image_paths = [data['file_name'] for data in datalist]
    image_cls_list = [image_path.split("/")[0] for image_path in image_paths]
    labels = [list(count_dict.keys()).index(image_cls) for image_cls in image_cls_list]
...
class ImageNetCustom(Dataset):
    def __init__(self, root_dir="/zhaobai/ImageNet-1K/train", subset="imagenet1k", transform=None, file_list="file_list.txt"):
        ...
        self.subset = subset
        self.count_dict = get_count_dict(subset)
...
class ImageNetGeneration(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        label = self.labels[idx]
        # image = Image.open(image_path).convert("RGB")
        class_name = list(self.count_dict.keys())[int(label)]
...
def get_generation_dataset(root, split="train",subset='NONE',filelist="file_list.txt"):
